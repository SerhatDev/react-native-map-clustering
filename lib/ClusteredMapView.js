...
import { Dimensions, LayoutAnimation, Platform, Text, Image } from "react-native";
  ...
  const generateSpiral = (count, centerLocation) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      res[i] = {
        longitude,
        latitude,
        markerType: clusterChildren[i].properties.markerType,
  ...
  return (
    <MapView
      ...
      {spiderMarkers.map(marker => (
        marker.markerType == "0" ?
        <Marker
          key={"n"+marker.latitude}
          coordinate={marker}
          onPress={marker.onPress}
        ><Image source={require('../../../assets/MinLocNorm.png')} style={{ height: 40, width: 40 }} resizeMode="contain" /></Marker>
        :
        <Marker
          key={"p"+marker.latitude}
          coordinate={marker}
          onPress={marker.onPress}
        ><Image source={require('../../../assets/MinLocGigaPrime.png')} style={{ height: 40, width: 40 }} resizeMode="contain" /></Marker>
